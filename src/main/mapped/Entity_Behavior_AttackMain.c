/**
 * @brief Entity attack main: entity[6] switch, attack states
 * @note Original: func_8006C0C4 at 0x8006C0C4
 */
// Entity_Behavior_AttackMain



/* WARNING: Globals starting with '_' overlap smaller symbols at the same address */

void FUN_8006c0c4(undefined1 *param_1)

{
  char cVar1;
  short sVar2;
  undefined2 uVar3;
  int iVar4;
  
  switch(param_1[6]) {
  case 0:
    FUN_8006bdf0(param_1,0);
    cVar1 = param_1[6];
    goto LAB_8006c25c;
  case 1:
    param_1[6] = param_1[6] + '\x01';
    if (param_1[0x5e] == '\0') {
      FUN_80074590(0xb,0xfffffffe,0);
      *(undefined2 *)(param_1 + 0x40) = 4;
    }
    else {
      FUN_80074590(0xb,0xfffffffc,0);
      *(undefined2 *)(param_1 + 0x40) = 2;
    }
    break;
  case 2:
    sVar2 = *(short *)(param_1 + 0x40);
    *(short *)(param_1 + 0x40) = sVar2 + -1;
    if (sVar2 == 1) {
      *(undefined2 *)(param_1 + 0x80) = 100;
      *(undefined2 *)(param_1 + 0x82) = 200;
      *(undefined2 *)(param_1 + 0x84) = 0xa0;
      *(undefined2 *)(param_1 + 0x86) = 0x104;
      *param_1 = 1;
      param_1[6] = param_1[6] + '\x01';
      if (param_1[0x5e] == '\0') {
        uVar3 = 8;
      }
      else {
        uVar3 = 7;
      }
      *(undefined2 *)(param_1 + 0x40) = uVar3;
      *(undefined2 *)(param_1 + 0x42) = 3;
      FUN_8006bd20(param_1);
    }
    break;
  case 3:
    if ((param_1[0x2b] == '\x01') &&
       (sVar2 = *(short *)(param_1 + 0x42), *(short *)(param_1 + 0x42) = sVar2 + -1, sVar2 == 1)) {
      *(undefined2 *)(param_1 + 0x42) = 3;
      param_1[0x2b] = 0;
    }
    FUN_8006be88(param_1);
    if ((byte)param_1[0x2b] < 2) goto LAB_8006c2a8;
    if (param_1[0x2b] == 2) {
      DAT_800e7ef9 = 2;
    }
    *(undefined2 *)(param_1 + 0x40) = 10;
    cVar1 = param_1[6];
    *(undefined2 *)(param_1 + 0x42) = 0x155;
LAB_8006c25c:
    param_1[6] = cVar1 + '\x01';
    break;
  case 4:
    if (*(short *)(param_1 + 0x42) < 0x800) {
      iVar4 = FUN_80083e80();
      _DAT_800e7eb0 = _DAT_800e7eb0 + iVar4 * -0x100;
      *(short *)(param_1 + 0x42) = *(short *)(param_1 + 0x42) + 0x155;
    }
LAB_8006c2a8:
    sVar2 = *(short *)(param_1 + 0x40);
    *(short *)(param_1 + 0x40) = sVar2 + -1;
    if (sVar2 == 1) {
      DAT_800e807e = 0;
      *param_1 = 2;
      param_1[4] = 3;
      if ((DAT_800e7fc6 & 1) != 0) {
        DAT_800e7ef9 = 0;
      }
    }
  }
  if (DAT_800e7fc6 == 0) {
    DAT_800e807e = 0;
    *param_1 = 2;
    param_1[0x2b] = 2;
    param_1[4] = 2;
  }
  return;
}
