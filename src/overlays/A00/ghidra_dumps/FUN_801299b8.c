// FUN_801299b8

/* WARNING: Globals starting with '_' overlap smaller symbols at the same address */

void FUN_801299b8(int param_1)

{
  undefined2 uVar1;
  ushort uVar2;
  short sVar3;
  int iVar4;
  
  if (*(char *)(param_1 + 6) == '\0') {
    uVar2 = *(short *)(param_1 + 0x40) - 1;
    *(ushort *)(param_1 + 0x40) = uVar2;
    if ((int)((uint)uVar2 << 0x10) < 1) {
      *(char *)(param_1 + 6) = *(char *)(param_1 + 6) + '\x01';
      FUN_80132a30();
      return;
    }
  }
  else {
    if (*(char *)(param_1 + 6) != '\x01') {
      FUN_80132a30();
      return;
    }
    if ((_DAT_1f80017c & 0xf) == 0) {
      iVar4 = (int)*(short *)(param_1 + 0x7a);
      *(undefined2 *)(param_1 + 0xb8) = *(undefined2 *)(iVar4 * 6 + -0x7feb597c);
      *(undefined2 *)(param_1 + 0xba) = *(undefined2 *)(iVar4 * 6 + -0x7feb597a);
      uVar1 = *(undefined2 *)(iVar4 * 6 + -0x7feb5978);
      *(ushort *)(param_1 + 0x7a) = *(short *)(param_1 + 0x7a) + 1U & 7;
      *(undefined2 *)(param_1 + 0xbc) = uVar1;
    }
  }
  sVar3 = FUN_801332c4(param_1);
  if ((sVar3 == 0) && (sVar3 = FUN_80133700(param_1), sVar3 == 1)) {
    FUN_80133610(param_1,0);
    *(undefined1 *)(param_1 + 5) = 3;
    *(undefined1 *)(param_1 + 6) = 0;
    *(undefined2 *)(param_1 + 0x6c) = 0;
  }
  return;
}

