// FUN_08030c84

/* WARNING: Control flow encountered bad instruction data */
/* WARNING: Globals starting with '_' overlap smaller symbols at the same address */

void FUN_08030c84(undefined1 *param_1)

{
  byte bVar1;
  int iVar2;
  
  bVar1 = param_1[4];
  if (bVar1 == 1) {
    param_1[1] = 1;
    func_0x00077e7c(param_1);
    func_0x00041098(param_1);
    if (param_1[5] == '\0') {
      if (param_1[0x2b] == '\x03') {
        *param_1 = 2;
        func_0x00040d68(param_1,0x80135a24);
        param_1[5] = 1;
        param_1[6] = 0;
        param_1[0x70] = 1;
                    /* WARNING: Bad instruction - Truncating control flow here */
        halt_baddata();
      }
    }
    else {
      if (param_1[5] != '\x01') {
        param_1[0x2b] = 0;
                    /* WARNING: Bad instruction - Truncating control flow here */
        halt_baddata();
      }
      if (param_1[0x71] == -1) {
        param_1[4] = 2;
      }
      if (param_1[0x5e] != '\0') {
        func_0x0004190c(param_1);
      }
      if (param_1[6] == '\x01') {
        DAT_800e806c = 8;
        param_1[6] = param_1[6] + '\x01';
        halt_baddata();
      }
      if (param_1[6] != '\x02') {
        param_1[0x2b] = 0;
        halt_baddata();
      }
      func_0x0006cba8(param_1 + 0x2c);
      _DAT_800e7ed6 =
           func_0x00085690((int)_DAT_1f800164 - (int)*(short *)(param_1 + 0x36),
                           (int)*(short *)(param_1 + 0x2e) - (int)_DAT_1f800160);
      _DAT_800e7ed6 = _DAT_800e7ed6 & 0xfff;
    }
    param_1[0x2b] = 0;
    func_0x000518fc(param_1);
  }
  else if (bVar1 < 2) {
    if (bVar1 == 0) {
      iVar2 = func_0x000519e0(param_1,0xf,_DAT_800ecfec,0x80131b48);
      if (iVar2 != 0) {
        return;
      }
      *(undefined4 *)(param_1 + 0x3c) = _DAT_800ecf90;
      func_0x00040cdc(param_1,0x8014005c,0x801359fc);
      func_0x00041718(param_1,0x15,0);
      *(undefined2 *)(param_1 + 0x56) = 0x140;
      param_1[0xb] = 0x40;
      *param_1 = 9;
      *(undefined2 *)(param_1 + 0x80) = 0xbe;
      *(undefined2 *)(param_1 + 0x82) = 0x17c;
      *(undefined2 *)(param_1 + 0x84) = 0x8c;
      *(undefined2 *)(param_1 + 0x86) = 0x104;
      param_1[0x70] = 1;
      param_1[0x2b] = 0;
      param_1[0x5e] = 0;
      *(undefined2 *)(param_1 + 0x54) = 0;
      *(undefined2 *)(param_1 + 0x56) = 0x800;
      *(undefined2 *)(param_1 + 0x58) = 0;
      param_1[4] = param_1[4] + '\x01';
    }
  }
  else if (bVar1 == 2) {
    param_1[4] = 3;
  }
  else if (bVar1 == 3) {
    func_0x0007a624(param_1);
    return;
  }
                    /* WARNING: Bad instruction - Truncating control flow here */
  halt_baddata();
}

