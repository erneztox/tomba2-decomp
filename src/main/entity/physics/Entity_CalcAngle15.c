...
#include "tomba.h"
s32 Entity_CalcAngle15(int param_1,u8 *param_2,int param_3)

{
  ...
  iVar5 = (int)(((uint)param_1->pos_z - (uint)param_2->pos_z) * 0x10000)
          >> 0x10;
  uVar2 = Math_SqrtGTE(iVar6 * iVar6 + iVar5 * iVar5);
  bVar1 = false;
  if (((int)(uint)uVar2 <= (int)param_1->bounds_min_x + (int)param_2->bounds_min_x) &&
    ...
    _g_AngleTarget = Math_Atan2(-iVar5,iVar6);
    bVar1 = true;
  }
  uVar3 = 0;
  if (bVar1) {
    if ((*param_2 & 8) == 0) {
      ...
      uVar4 = System_Main(param_1,param_2,0,uVar3);
    }
    else {
      uVar4 = Entity_InitFull(param_1,param_2,0,0);
    }
    uVar3 = 1;
    if ((uVar4 & 0x80) == 0) {
      g_State182 = 0;
