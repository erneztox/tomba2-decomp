// FUN_80105720

/* WARNING: Globals starting with '_' overlap smaller symbols at the same address */

void FUN_80105720(byte *param_1,int param_2)

{
  short sVar1;
  short sVar2;
  short sVar3;
  ushort uVar4;
  int iVar5;
  int iVar6;
  uint uVar7;
  
  if (((*(ushort *)(param_1 + 0x17e) & 0x200) == 0) &&
     (iVar5 = func_0x8002300c(param_1,param_2,(int)*(short *)(param_2 + 0x80) << 2), iVar5 != 0)) {
    uVar7 = (uint)_DAT_1f80009c;
    *(undefined1 *)(param_2 + 0x2b) = 1;
    if (DAT_1f800137 == '\0') {
      uVar4 = *(ushort *)(param_1 + 0x140);
      iVar6 = func_0x80085690(-((int)(((uint)*(ushort *)(param_2 + 0x36) -
                                      (uint)*(ushort *)(param_1 + 0x36)) * 0x10000) >> 0x10),
                              (int)(((uint)*(ushort *)(param_2 + 0x2e) -
                                    (uint)*(ushort *)(param_1 + 0x2e)) * 0x10000) >> 0x10);
      iVar5 = (int)((iVar6 * 0x10 + (uVar4 - 0x400) * -0x10) * 0x10000) >> 0x10;
      if (iVar5 < 0) {
        iVar5 = -iVar5;
      }
      if (iVar5 < 0x1000) {
        _DAT_1f80018e = 0x4101;
        DAT_800bf840 = 0x42;
        _DAT_1f80018c = *(short *)(param_1 + 0x140) + -0x400;
        FUN_8010e7fc();
        return;
      }
      iVar5 = (int)((iVar6 * 0x10 + (*(ushort *)(param_1 + 0x140) + 0x400) * -0x10) * 0x10000) >>
              0x10;
      if (iVar5 < 0) {
        iVar5 = -iVar5;
      }
      if (iVar5 < 0x1000) {
        _DAT_1f80018c = (short)(*(ushort *)(param_1 + 0x140) + 0x400);
        _DAT_1f80018e = 0x4201;
        DAT_800bf840 = 0x43;
      }
    }
    if ((((int)(uint)_DAT_1f80008c <=
          (int)*(short *)(param_1 + 0x80) + (int)*(short *)(param_2 + 0x80) + 0x50) &&
        (*(undefined1 *)(param_2 + 0x2b) = 2,
        (int)(uint)_DAT_1f80008c <=
        (int)*(short *)(param_1 + 0x80) + (int)*(short *)(param_2 + 0x80))) && ((*param_1 & 4) == 0)
       ) {
      iVar6 = (int)((uVar7 + 0x800) * 0x10000) >> 0x10;
      iVar5 = func_0x80083f50(iVar6);
      sVar1 = *(short *)(param_1 + 0x80);
      sVar2 = *(short *)(param_2 + 0x80);
      iVar6 = func_0x80083e80(iVar6);
      sVar3 = *(short *)(param_2 + 0x80);
      *(short *)(param_1 + 0x2e) =
           *(short *)(param_2 + 0x2e) + (short)(iVar5 * ((int)sVar1 + (int)sVar2) >> 0xc);
      *(short *)(param_1 + 0x36) =
           *(short *)(param_2 + 0x36) -
           (short)(iVar6 * ((int)*(short *)(param_1 + 0x80) + (int)sVar3) >> 0xc);
    }
  }
  return;
}

