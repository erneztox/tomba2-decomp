...
#include "tomba.h"
s32 Entity_State_Jump4(int param_1,int param_2)

{
  ...
  if (bVar1 == 1) {
    ...
    if (param_1->timer1 == 0xe) {
      iVar4 = Entity_SpawnNoPos(0x20,0);
      if (iVar4 != 0) {
      ...
      if (param_1->draw_y != 2) {
        Entity_SetSubEntityData(param_1->sprite_ptr1,1,0);
      }
    ...
    if (bVar1 == 0) {
      if (DAT_800bf80a == '\0') {
        ...
        DAT_800e7e86 = 0;
        Entity_LoadAnimIfChanged(&g_CollisionEntity,0xcc,0);
        param_1->timer1 = 0x14;
        param_1->action_state = param_1->action_state + '\x01';
      }
      Entity_UpdateMatrix(param_1);
    ...
    if (bVar1 == 2) {
      ...
      uVar3 = 0;
      if (sVar2 == -1) {
        Entity_LoadAnimIfChanged(&g_CollisionEntity,2,0);
        uVar3 = 2;
      }
    }
  }
