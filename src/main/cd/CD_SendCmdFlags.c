...
#include "tomba.h"
void CD_SendCmdFlags(uint param_1)

{
  u8 local_10->counter1;
  
  local_10->type = (u8)param_1;
  CD_SendCmdRetry(0xe,local_10,0);
  if ((param_1 & 0x100) != 0) {
    _DAT_80102760 = (uint)((param_1 & 0x20) == 0);
    CD_SetTimerCallback(CD_AllocBuffer);
    CD_SetErrorCallback(MDEC_Process);
  }
  CD_SendCmdRetry(0x1b,0,0);
  return;
}
