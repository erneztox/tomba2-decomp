/**
 * @brief Spawns drop item: child entity 0x0D with timed lifecycle, timer 0x7C7E, SFX 8, manages global ptr
 * @note Original: func_80072114 at 0x80072114
 */
// Entity_SpawnDropItem



/* WARNING: Globals starting with '_' overlap smaller symbols at the same address */

#include "tomba.h"
void Entity_SpawnDropItem(s32 param_1,s16 param_2)

{
  int iVar1;
  
  iVar1 = Entity_SpawnChild(param_1,0x80,3,0xd);
  if (iVar1 != 0) {
    iVar1->callback = Entity_Behavior_TimedLife;
    iVar1->sub_type = 2;
    iVar1->anim_timer = param_2;
    iVar1->draw_y = 0x7c7e;
    Audio_PlaySoundEffect(8,5,0);
    if (_DAT_800bf83c != 0) {
      _DAT_800bf83c->state = STATE_DEAD;
    }
    _DAT_800bf83c = iVar1;
    if (_g_FrameCounter < (s16)(u16)g_ItemCount) {
      iVar1->sub_action = 1;
    }
    else {
      iVar1->sub_action = 0;
    }
  }
  return;
}
