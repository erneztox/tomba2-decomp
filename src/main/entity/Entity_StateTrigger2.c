...
#include "tomba.h"
s32 Entity_StateTrigger2(int param_1)

{
  ...
  if (bVar1 == 1) {
    ...
    if ((uVar2 & 0x2000) == 0) {
      if ((uVar2 & 0x1000) == 0) {
        iVar3 = Entity_LoadSprite2(param_1,uVar2 & 0x3ff);
      }
      else {
        iVar3 = Entity_LoadSprite(param_1,uVar2 & 0x3ff);
      }
    }
    else {
      iVar3 = Entity_SpawnChildren(param_1,uVar2 & 0x3ff);
    }
    _DAT_800bf844 = iVar3;
    if (iVar3 != 0) {
    ...
    Entity_SetAnimMode(param_1,(int)param_1->event_param,4);
  }
  else {
    if (bVar1 != 2) {
      if (bVar1 != 3) {
    ...
    Entity_SetAnimMode(param_1,(int)*(s16 *)(param_1 + 0x76),4);
    if ((param_1->event_id & 0x800) != 0) {
      return 1;
    }
  }
