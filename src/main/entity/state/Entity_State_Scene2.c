...
#include "tomba.h"
s32 Entity_State_Scene2(int param_1)

{
  ...
  case 0:
    Entity_SetSubState22(&g_CollisionEntity,0xcf,2);
    uVar1 = param_1->event_id;
    if ((uVar1 & 0x2000) == 0) {
      if ((uVar1 & 0x1000) == 0) {
        _DAT_800bf844 = Entity_LoadSprite2(param_1,uVar1 & 0x3ff);
      }
      else {
        _DAT_800bf844 = Entity_LoadSprite(param_1,uVar1 & 0x3ff);
      }
    }
    else {
      _DAT_800bf844 = Entity_SpawnChildren(param_1,uVar1 & 0x3ff);
    }
    if (_DAT_800bf844 != 0) {
    ...
    if ((param_1->event_id & 0x800) == 0) {
      UI_DrawItemIcon(param_1->event_id & 0x3ff,4);
      Inventory_RemoveItem3(param_1->event_id & 0x3ff,1);
    }
    break;
  case 1:
...
LAB_800448d4:
    Entity_SetAnimMode(param_1,iVar2,4);
    break;
  case 2:
    ...
    Entity_SetSubState22(&g_CollisionEntity,2,4);
    break;
  case 3:
